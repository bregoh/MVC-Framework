...
class UserAgent {
	...
	public function parseUserAgentString($userAgent) {
		...
		elseif ($this->mypreg_match('%Mozilla/[0-9.]* \((?:iPhone|iPad); .*\) AppleWebKit/[0-9.]*\+{0,1} {1,2}\(KHTML, like Gecko\) Version/([0-9.]*) .*Safari/[0-9.]*%i', $userAgent, $matches)) {
			...
			$this->rawVersion['major'] = current($t);
			$this->rawVersion['minor'] = next($t);
			$this->rawVersion['build'] = next($t);
			$this->rawVersion['rev'] = next($t);

			$browserVersion = current($t) . "." . next($t) . next($t);
	...
	function processAndroidVersion($version) {
		if (preg_match('/\A[0-9.]*\Z/i', $version)) {
			...
			$count = count($androidVersionPieces);
			if($count == 1)
			{
				$androidVersion = floatval($androidVersionPieces[0]);
			}
			else
			{
